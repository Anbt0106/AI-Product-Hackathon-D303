...
tự trả lời Micro-Check. Cả ba bước AI trung tâm đều gọi Gemini thật:
...
- Không gửi toàn bộ data pack hoặc chatlog lên Gemini.
...
Các endpoint dùng cùng cấu hình `GEMINI_API_KEY` và `GEMINI_MODEL` trong
...
- Thiếu nguồn: chặn phía client, không gọi Gemini.
...
4. Thiếu nguồn không tạo lời gọi Gemini.
