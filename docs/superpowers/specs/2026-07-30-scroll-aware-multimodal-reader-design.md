...
trước khi gọi OpenAI Responses API.
...
- `ai-error`: OpenAI không xử lý được request multimodal.
...
- Cập nhật `codebase/server.mjs` để validate ảnh và tạo Responses API input có
