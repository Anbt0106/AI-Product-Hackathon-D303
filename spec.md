...
| Grounding Gate, Scope Guard | Thật, luật xác định | `codebase/engine/grounding-gate.js`, `codebase/engine/scope-guard.js` |
| **Mastery Classifier — quyết định trung tâm** | **AI thật khi server có key**; model mặc định `gemini-3.5-flash-lite`; fallback rule phải ghi trace | `codebase/server.mjs`, `codebase/engine/ai-client.js` |
